...
namespace Pathfinding {
	...
	public abstract class AIBase : VersionedMonoBehaviour {
		...
		protected virtual void Start () {
			mindist = FindObjectOfType<PatrolEnemy2>().GetMinDist();
			target1 = FindObjectOfType<PatrolEnemy2>().GetTrans();
			target2 = FindObjectOfType<PatrolEnemy2>().GetTarget();
			startHasRun = true;
			Init();
		}
		...
		protected virtual void Update () {
			...
			if (dist > mindist)
			{
				FindObjectOfType<PatrolEnemy2>().Do();
				return;
			}
			if (shouldRecalculatePath) SearchPath();
